...
esp_err_t cmd_handler(httpd_req_t *req)
{
    ...
    if (buf_len > 1) {
        ...
        if (httpd_req_get_url_query_str(req, buf, buf_len) == ESP_OK) {
            ...
            } else if (httpd_query_key_value(buf, "slider", angle, sizeof(angle)) == ESP_OK) {
                receive_angle = true;
            } else {
                free(buf);
    ...
    } else if (!strcmp(variable, "clip_open")) {
        uart.send(ESP32_CMD_ANGLE, ':', "90");        
    } else if (!strcmp(variable, "clip_close")) {
        uart.send(ESP32_CMD_ANGLE, ':', "0");
